  ...
  private func drawTheDots() {
    ...
    for (index,color) in colors.enumerated() {
      ...
      if index == colors.count - 2 || index == colors.count - 1{
          view = UIView(frame: CGRect(x: ((spacing + dotSize) * CGFloat(index)), y: (frame.height/2) - (dotSize/2) , width: 0, height: 0))
      }else{
          view = UIView(frame: CGRect(x: ((spacing + dotSize) * CGFloat(index)), y: (frame.height/2) - (dotSize/2) , width: dotSize, height: dotSize))
      }
