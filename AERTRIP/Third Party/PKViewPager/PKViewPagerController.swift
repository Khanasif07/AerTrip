...
public extension PKViewPagerControllerDataSource {
...
public class PKViewPagerController:UIViewController {
    
    public weak var dataSource:PKViewPagerControllerDataSource!
    public weak var delegate:PKViewPagerControllerDelegate?
    ...
    fileprivate var tabsList = [PKViewPagerTab]()
    fileprivate var tabsViewList = [PKViewPagerTabView]()
    
    fileprivate var isIndicatorAdded = false
    fileprivate var currentPageIndex = 0
    
    public var options:PKViewPagerOptions!
    ...
    fileprivate func setupTabContainerView() {
        ...
        // Adding Gesture
        let tabViewTapGesture = UITapGestureRecognizer(target: self, action: #selector(PKViewPagerController.tabContainerTapped(_:)))
    ...
    fileprivate func setupTabs() {
        ...
        if options.fitAllTabsInView {
            ...
            for (index,eachTab) in tabsList.enumerated() {
                ...
                let tabView = PKViewPagerTabView()
                tabView.frame = CGRect(x: xPosition, y: 0, width: eachLabelWidth, height: options.tabViewHeight)
                tabView.setup(tab: eachTab, options: options, condition: PKViewPagerTabView.SetupCondition.fitAllTabs)
            ...
            for (index,eachTab) in tabsList.enumerated() {
                ...
                tabView.setup(tab: eachTab, options: options, condition: PKViewPagerTabView.SetupCondition.distributeNormally)
...
extension PKViewPagerController: UIPageViewControllerDelegate {
...
extension PKViewPagerController:UIPageViewControllerDataSource {
