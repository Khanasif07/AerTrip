                ...
                if let events = onData[date] as? [AccountDetailEvent] {
                    ...
                    let fltrd = events.filter { event in
                        return ((event.title.lowercased().contains(forText.lowercased())) || (event.voucherNo.lowercased().contains(forText.lowercased())) ||
                                    ...
                                    (self.removeSpecialChar(from:event.flightNumber).contains(self.removeSpecialChar(from: forText))) || (self.removeSpecialChar(from:event.voucher.rawValue).contains(self.removeSpecialChar(from: forText))) ||  (self.removeSpecialChar(from:"\(event.amount)").contains(self.removeSpecialChar(from: forText))) || (event.transactionDate.lowercased().contains(forText.lowercased())))
//                                    ||
