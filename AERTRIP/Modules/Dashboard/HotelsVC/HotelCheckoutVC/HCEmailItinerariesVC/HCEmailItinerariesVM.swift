...
class HCEmailItinerariesVM {
    ...
    var flightTraveller: [T] = []
    var isForFlight:Bool = false
    var emailItinerariesAPIGroup = DispatchGroup()
    var multipleEmailSentFail = false
    ...
    func sendEmailIdApi(emailId: [String], isMultipleEmailSending : Bool, currentEmailIndex: Int) {
        multipleEmailSentFail = false
        self.emailItinerariesAPIGroup.notify(queue: .main) { [weak self] in
            guard let sSelf = self else {return}
            if sSelf.multipleEmailSentFail {
            ...
            printDebug("multi email sent..")
        }
        
        for email in emailId {
            sendEmailIdApi(emailId: email, isMultipleEmailSending: isMultipleEmailSending, currentEmailIndex: currentEmailIndex)
        }
    }
    ...
    func sendEmailIdApi(emailId: String, isMultipleEmailSending : Bool, currentEmailIndex: Int) {
        ...
        params["\(APIKeys.email.rawValue)[]"] = emailId
        if isMultipleEmailSending {
            emailItinerariesAPIGroup.enter()
        }
        APICaller.shared.sendDashBoardEmailIDAPI(bookingID: self.bookingId, params: params, loader: false) { [weak self] (success, errors, recentSearchesHotels) in
            guard let sSelf = self else {
                if isMultipleEmailSending {
                    self?.emailItinerariesAPIGroup.leave()
                }
                return }
            if success {
                if !isMultipleEmailSending {
                ...
                sSelf.multipleEmailSentFail = true
            }
            if isMultipleEmailSending {
                sSelf.emailItinerariesAPIGroup.leave()
            }
        }
    }
    ...
    func fillData() {
        for traveller in self.flightTraveller{
            var emailInfo = HCEmailItinerariesModel()
            emailInfo.emailId = traveller.email.first?.value ?? ""
            emailInfo.emailStatus = .toBeSend
            self.emailInfo.append(emailInfo)
        }
    }
