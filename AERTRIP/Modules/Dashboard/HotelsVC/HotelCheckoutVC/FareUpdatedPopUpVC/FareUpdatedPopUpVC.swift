...
class FareUpdatedPopUpVC: BaseVC {
    ...
    private func updateDecreasePopUp(decreasedAmount: Double) {
        setupDecreasePopUp()
        //let currency = "$"
        let attrText = AppGlobals.shared.getTextWithImage(startText: "", image: #imageLiteral(resourceName: "ic_fare_dipped"), endText: "   \(LocalizedString.FareDippedBy.localized)  \(decreasedAmount.getConvertedAmount(using: AppFonts.Regular.withSize(16)).string)", font: AppFonts.Regular.withSize(16.0))
