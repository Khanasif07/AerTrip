...
- (void)baseInit
{
   ...
   // _fullSelectedImage = nil;
    ...
    _rating = 0;
    _isEditable = NO;
    _enableHalfRatings = NO;
    _enableFlaotRatings = NO;
    _delegate = nil;
...
- (void)handleTouchAtLocation:(CGPoint)touchLocation
{
    ...
    for (NSInteger i = self.emptyImageViews.count - 1; i >= 0; i--) {
        ...
        if (touchLocation.x > imageView.frame.origin.x) {
            ...
            if ([imageView pointInside:newLocation withEvent:nil] && (self.enableFlaotRatings || self.enableHalfRatings)) {
                CGFloat decimalNum = newLocation.x/imageView.frame.size.width;
                newRating = i + decimalNum;
                if (self.enableHalfRatings) {
