...
//        }
    ...
    struct HotelFormPreviosSearchData: Codable {
        ...
        private enum HotelFormKeys: String, CodingKey {
            case hotelFormRoomNumber
            case hotelFormAdultsCount
            case hotelFormChildrenAge
            case hotelFormCheckInDate
            case hotelFormCheckOutDate
            case hotelFormDestId
            case hotelFormDestType
            case hotelFormDestName
            case hotelFormRatingCount
        }
        
        init(from decoder:Decoder) throws {
            let values = try decoder.container(keyedBy: HotelFormKeys.self)
            self.roomNumber = try values.decode(Int.self, forKey: .hotelFormRoomNumber)
            self.adultsCount = try values.decode([Int].self, forKey:  .hotelFormAdultsCount)
            self.childrenCounts  = try values.decode([Int].self, forKey: .hotelFormChildrenAge)
            self.childrenAge = try values.decode([[Int]].self, forKey: .hotelFormCheckInDate)
            self.checkInDate = try values.decode(String.self, forKey: .hotelFormCheckOutDate)
            self.checkOutDate = try values.decode(String.self, forKey: .hotelFormDestId)
            self.destId = try values.decode(String.self, forKey: .hotelFormDestType)
            self.destType = try values.decode(String.self, forKey: .hotelFormDestName)
            self.destName = try values.decode(String.self, forKey: .hotelFormRatingCount)
            self.ratingCount = try values.decode([Int].self, forKey: .hotelFormAdultsCount)
        }
    }

    var hotelFormData: HotelFormPreviosSearchData? {
        get {
            if let obj = UserDefaults.standard.retrieve(object: UserInfo.HotelFormPreviosSearchData.self , fromKey: APIKeys.hotelFormPreviosSearchData.rawValue) {
                return obj
            }
            return nil
        }
        set {
            if let vlaue = newValue {
                UserDefaults.standard.save(customObject: vlaue, inKey: APIKeys.hotelFormPreviosSearchData.rawValue)
            }
            else{
