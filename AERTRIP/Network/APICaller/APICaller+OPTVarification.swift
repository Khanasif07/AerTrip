...
extension APICaller{
    ...
    func validateEnableDisableWalletOtp(params: JSONDictionary, completionBlock: @escaping(_ success: Bool, _ errorCodes: ErrorCodes)->Void ){
        AppNetworking.POST(endPoint: .validateDisableOtp,parameters: params,success: {[weak self] data in
            guard let self = self else {return}
            self.handleResponse(data) { (success, data) in
                completionBlock(true, [])
            } failure: { (errorCode) in
    ...
    func cancelEnableDisableWalletOtp(params: JSONDictionary, completionBlock: @escaping(_ success: Bool, _ errorCodes: ErrorCodes)->Void ){
        AppNetworking.POST(endPoint: .cancelWalletEnableDisable,parameters: params,success: {[weak self] data in
            guard let self = self else {return}
            self.handleResponse(data) { (success, data) in
                completionBlock(true, [])
            } failure: { (errorCode) in
